...
public class BulletWorld extends BaseWorld<BulletEntity> {
	...
	public void add (final BulletEntity entity) {
		...
		if (entity.body != null) {
			...
			//entity.body.setContactCallbackFlag(btCollisionObject.CollisionFlags.CF_STATIC_OBJECT);
		//	entity.body.setContactCallbackFilter(btCollisionObject.CollisionFlags.CF_STATIC_OBJECT );
		}
	}
	public void add2 (final BulletEntity entity) {
		super.add(entity);
		if (entity.body != null) {
			if (entity.body instanceof btRigidBody)
				((btDiscreteDynamicsWorld)collisionWorld).addRigidBody((btRigidBody)entity.body);
			...
			// Store the index of the entity in the collision object.
			entity.body.setUserValue(entities.size - 1);
		//	entity.body.setContactCallbackFlag(btCollisionObject.CollisionFlags.CF_NO_CONTACT_RESPONSE);
		//	entity.body.setContactCallbackFilter(btCollisionObject.CollisionFlags.CF_NO_CONTACT_RESPONSE);
		}
	}
	...
	public void renderDebug (ModelBatch batch,PerspectiveCamera cam) {
		//if (renderMeshes) super.render(batch, lights, entities);
		if (debugDrawer != null && debugDrawer.getDebugMode() > 0) {
			batch.flush();
