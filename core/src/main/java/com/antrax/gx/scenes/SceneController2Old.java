...
public class SceneController2Old {
	...
	public ModelInstance rayTest(btCollisionWorld collisionWorld, Ray ray) {
...
*/
	...
	Camera camera; 
	public SceneController2Old(Camera camera) {
		this.camera = camera;
	}
    ...
    public void loadAssets(SceneManager sceneManager){
    	assetRobot = new GLTFLoader().load( Gdx.files.internal("models/robot2.gltf"), false);        
    ...
    void addBounding(Scene scene,boolean firstValid) {
    	...
    	if (firstValid) {
			...
			gameObjectParent.setModelInstanceMain(modelInstance);
    ...
    private void updateObjects(float delta) {
		for (UUID key : gameObjects.keySet()) 
			gameObjects.get(key).update(delta/100);
    }
